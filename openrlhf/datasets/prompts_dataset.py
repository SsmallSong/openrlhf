...
def preprocess_data(data, input_template=None, input_key=None, apply_chat_template=None) -> str:
    ...
    if input_key:
        if apply_chat_template:
            prompt_now = data[input_key]
            messages = [{"role": "user", "content": data[input_key]},]
                
            prompt = apply_chat_template(messages, tokenize=False, add_generation_prompt=True,return_tensors="pt")
            input_template = None
        else:
